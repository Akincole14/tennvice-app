...
import {
  LayoutDashboard, Users, Wrench, ShieldCheck,
  Star, BarChart3, ClipboardList, Settings, LogOut, X,
} from "lucide-react";
import { signOut } from "next-auth/react";
import { cn } from "@/lib/utils";

const mainNav = [
  { href: "/manager/dashboard",   label: "Dashboard",  icon: LayoutDashboard, exact: true  },
  { href: "/manager/customers",   label: "Customers",  icon: Users,           exact: false },
  { href: "/manager/technicians", label: "Technicians",icon: Wrench,          exact: false },
  { href: "/manager/reports",     label: "Reports",    icon: BarChart3,       exact: false },
];

const sheetItems = [
  { href: "/manager/admins",        icon: ShieldCheck,   label: "Admins"           },
  { href: "/manager/managers",      icon: Star,          label: "Managers"         },
  { href: "/manager/visit-reports", icon: ClipboardList, label: "Visit Reports"    },
  { href: "/manager/account",       icon: Settings,      label: "Account settings" },
];

export default function MobileManagerNav({ name, photo }: { name: string; photo?: string | null }) {
    ...
    <>
      ...
      <header className="md:hidden fixed top-0 inset-x-0 z-40 bg-white border-b border-gray-100 h-14 flex items-center px-4 justify-between">
        ...
        <button
          ...
          className="w-8 h-8 rounded-full overflow-hidden shrink-0 ring-2 ring-purple-100"
      ...
      >
        ...
        >
          <div className="px-6 py-4 flex items-center justify-between border-b border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-purple-600">Manager</p>
              </div>
            </div>
            <button
          ...
          <nav className="px-4 py-3 space-y-1">
            {sheetItems.map(({ href, icon: Icon, label }) => (
              <Link
                key={href}
                href={href}
                onClick={() => setOpen(false)}
