...
export default function ManagerCustomersClient({ customers: initial }: { customers: Customer[] }) {
  ...
  async function handleDelete(customerId: string, userId: string) {
    ...
    setDeletingId(null);
    if (res.ok) setCustomers(cs => cs.filter(c => c.id !== customerId));
  }
  ...
  return (
    <>
      ...
      <div className="md:hidden bg-white rounded-2xl border border-gray-200 divide-y divide-gray-100">
            ...
            <div key={c.id} className="px-4 py-4">
              ...
              <div className="flex items-center gap-3 text-xs text-gray-500 mt-1">
                ...
                <span className="ml-auto">
                  Last: {lastVisit ? new Date(lastVisit.scheduledAt).toLocaleDateString("en-GB", { day: "numeric", month: "short" }) : "none"}
                </span>
                <button
        ...
        <div className="px-4 py-3 text-xs text-gray-400">Showing {filtered.length} of {customers.length} customers</div>
      </div>
