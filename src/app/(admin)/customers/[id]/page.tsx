        ...
        include: {
          visits: {
            include: {
              report: {
                select: {
                  pipesCheck: true, heatingCheck: true, electricalCheck: true,
                  boilerCheck: true, followUpRequired: true, signedByTechnician: true,
                },
              },
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5 md:space-y-8 py-4 md:py-8">
      ...
      <div className="flex items-center justify-between gap-4">
        <Link href="/customers" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-900">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-1">
          Viewing as <span className="font-medium text-gray-700">{customer.user.name}</span>
      ...
      {nextVisit ? (
        <Link
          ...
          className="block bg-brand-600 text-white rounded-2xl p-5 hover:bg-brand-700 transition-colors"
        >
          ...
          {customer.emergencyCallouts > 0 && (
            <div className="flex justify-between col-span-2">
              ...
              <span className="font-medium text-gray-900">
                {customer.emergencyCallouts === Infinity ? "Unlimited" : customer.emergencyCallouts} per year
              </span>
            </div>
          )}
        </div>
      ...
      {/* Properties */}
      <div>
        <h2 className="font-semibold text-gray-900 mb-3">Properties</h2>
        <div className="space-y-3">
          {customer.properties.map((property) => {
            const lastCompleted = property.visits.find(
              (v) => v.status === "COMPLETED" && v.report?.signedByTechnician
            );
            ...
            const lastChecks = lastCompleted?.report
              ? [
                  ...
                  lastCompleted.report.heatingCheck,
                  lastCompleted.report.electricalCheck,
                  lastCompleted.report.boilerCheck,
                ].filter((c) => c !== "NOT_CHECKED")
              : [];
